...
class RoundSnapshotBatch(models.Model):
    """每轮结束本轮时写入一次的快照批次元数据。"""
    ...
    trigger = models.CharField(max_length=32, choices=TRIGGER_CHOICES, default='end_round')
...
class RoundSnapshotPlatform(models.Model):
    """轮次 × 平台维度汇总快照。"""
    ...
    clickbait_count_article_field = models.PositiveIntegerField(
        default=0, verbose_name='标题党篇数(Article字段)',
    )
    clickbait_count_by_rule = models.PositiveIntegerField(
        default=0, verbose_name='标题党篇数(规则重算)',
    )
    clickbait_count_unjudged = models.PositiveIntegerField(
        default=0, verbose_name='未判定篇数',
    )
    ...
    cycle_profit_record = models.ForeignKey(
    ...
    cycle_index = models.PositiveIntegerField(null=True, blank=True, verbose_name='周期序号')
    cycle_profit_total = models.DecimalField(
        max_digits=18, decimal_places=6, null=True, blank=True, verbose_name='周期利润',
    )
...
class RoundSnapshotWriter(models.Model):
    """轮次 × 写手维度汇总快照。"""

    round_num = models.PositiveIntegerField(db_index=True, verbose_name='模拟轮次')
    writer_account = models.CharField(max_length=64, db_index=True, verbose_name='写手账号')
    platform_id = models.IntegerField(verbose_name='平台编号')
    fan_count = models.PositiveIntegerField(default=0, verbose_name='粉丝数')
    round_revenue_total = models.DecimalField(
        max_digits=18, decimal_places=6, default=0, verbose_name='本轮最终收益合计',
    )
    round_revenue_raw = models.DecimalField(
        max_digits=18, decimal_places=6, default=0, verbose_name='本轮原始收益合计',
    )
    revenue_penalty_deduction = models.DecimalField(
        max_digits=18, decimal_places=6, default=0, verbose_name='收益惩罚扣减额',
    )
    revenue_penalty_article_count = models.PositiveIntegerField(
        default=0, verbose_name='收益惩罚文章数',
    )
    traffic_penalty_article_count = models.PositiveIntegerField(
        default=0, verbose_name='流量惩罚文章数',
    )
    health_score = models.IntegerField(default=100, verbose_name='健康分')
    health_tier = models.CharField(max_length=50, blank=True, default='', verbose_name='健康档位')
    push_coefficient = models.DecimalField(
        max_digits=5, decimal_places=4, default=Decimal('1.0000'), verbose_name='推流系数',
    )
...
class RoundSnapshotWriterFan(models.Model):
    """轮次 × 用户关注写手关系明细（粉丝列表）。"""

    round_num = models.PositiveIntegerField(db_index=True, verbose_name='模拟轮次')
    writer_account = models.CharField(max_length=64, db_index=True, verbose_name='写手账号')
    user_account = models.CharField(max_length=64, db_index=True, verbose_name='用户账号')
